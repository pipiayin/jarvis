...
zip pixnet.zip -r lambda_pixnet.py  pixnetTool.py  requests/  twlocation.py nocheckin.py
...
#aws --profile ailine lambda create-function --function-name pixnettravel --runtime python3.6 --role "arn:aws:iam::740157263594:role/sandyairun" --handler lambda_pixnet.lambda_travelhandler --timeout 59 --code "S3Bucket=sandyai2,S3Key=pixnet.zip"
...
aws --profile ailine lambda update-function-code --function-name pixnettravel --s3-bucket sandyai2 --s3-key pixnet.zip
